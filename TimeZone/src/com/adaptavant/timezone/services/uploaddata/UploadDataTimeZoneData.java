...
public class UploadDataTimeZoneData {
	Logger logger=Logger.getLogger(UploadDataTimeZoneData.class.getName());
	public void uploadTimeZoneData()
	{
		...
		try
		{
			...
			while(token.hasMoreTokens())
			{
				...
				String cityCode=token.nextToken().replace("\"", "").toUpperCase();
				cityCode=Normalizer
				...
				timezoneJDO.setTimeZoneName(WordUtils.capitalizeFully(timeZone.substring(timeZone.indexOf('/')+1)));
